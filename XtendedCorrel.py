# from https://github.com/PaulVanDev/HoeffdingD
...
def hoeffding(*arg):
    ...
    if len(arg) == 1:
        if isinstance(arg[0], pd.DataFrame):
            if arg[0].shape[0] > 1:
                return arg[0].apply(
                    lambda x: arg[0].apply(lambda y: hoeffding(x.values, y.values))
                )
    else:
        if len(arg) == 2:
            if type(arg[0]) is not np.ndarray:
                if len(arg[0].shape) > 1:
                    return print(
                        "ERROR inputs : hoeffding(df >2col) or hoeffding(numpy.array -1d- ,numpy.array -1d-)"
                    )
            ...
            if len(np.unique(x)) > 50:
                ...
                )  # faster strategy='quantile' but less accurate
            ...
            D2 = np.sum(
                np.multiply(
                    np.multiply((R - 1), (R - 2)), np.multiply((S - 1), (S - 2))
                )
            )
